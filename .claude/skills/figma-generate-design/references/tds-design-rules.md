# TDS Design Rules — figma-generate-design 오버레이
...
- `search_design_system`에서 `includeLibraryKeys` 사용 시 위 fileKey로 TDS 전용 검색 가능
...
TDS는 shadcn/ui 기반. 스크린 생성 시 아래 컴포넌트를 `search_design_system`으로 찾아 인스턴스 사용:
