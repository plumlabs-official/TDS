---
name: tds-make-component
description: "참조 프레임 → TDS 컴포넌트 자동 생성. 기존 TDS 인스턴스 재사용, 텍스트 스타일/변수 바인딩 복사, 프로퍼티 자동 정의. Use when user says '/make-component', '컴포넌트 만들기', 'make component', '컴포넌트 생성'."
---

# TDS Make Component

프로덕트 파일의 참조 프레임을 분석하여 TDS 라이브러리에 정규 컴포넌트를 생성합니다.

**MANDATORY**: load `figma-use` before every `use_figma` call.
**Always pass `skillNames: "tds-make-component"` when calling `use_figma`.**
...
1. **기존 TDS 인스턴스 재사용** — 절대 기본 도형으로 재생성하지 않는다. `importComponentByKeyAsync`로 가져온다.
...
> **주의**: Phase 1은 프로덕트 파일 대상, Phase 3은 TDS 파일(`H36eNEd6o7ZTv4R7VcyLf2`) 대상. fileKey를 혼동하지 않는다.
...
2. 각 INSTANCE의 `componentKey` → TDS 소속 여부 확인
3. TDS 파일에 동명 컴포넌트 존재 여부 검색
4. TDS 파일에서 관련 컴포넌트 그룹 위치 파악 (배치용)
...
### Phase 3: Creation (use_figma on TDS file)
...
| Step | 작업 | 검증 |
|------|------|------|
| a | 컴포넌트 프레임 생성 + auto-layout 설정 | `get_metadata`로 구조 확인 |
| b | TDS 인스턴스 import + 배치 (`importComponentByKeyAsync`) | return으로 ID 확인 |
| c | 인스턴스 componentProperties 설정 (참조에서 복사) | — |
| d | 텍스트 노드 생성 + textStyleId + boundVariables 바인딩 | — |
| e | 레이아웃 프레임 생성 + 설정 (중간 래퍼 등) | — |
| f | 컴포넌트 width, layoutSizing 설정 | — |
| g | 컴포넌트 프로퍼티 정의 추가 (TEXT, BOOLEAN, INSTANCE_SWAP) | — |
| h | componentPropertyReferences로 프로퍼티 ↔ 자식 노드 연결 | — |
| i | 생성된 모든 노드 ID 반환 | `get_screenshot` 시각 확인 |
...
1. **시각 검증** — `get_screenshot`으로 새 컴포넌트 캡처, 참조 프레임 스크린샷과 비교
2. **네이밍 검증** — naming-policy.md 기준:
   - 컴포넌트명: Title Case 공백
   - 레이어명: 허용 어휘, 금지 접미사 없음
   - 자동 생성명 없음
3. **프로퍼티 검증** — qa-rubric R4 기준:
   - Boolean: `Show`/`Is`/`Has` 접두어 (Figma display name은 Title Case 공백)
   - Variant key: camelCase
   - Variant value: lowercase
   - 프로퍼티 20개 이하
4. **구조 검증** — qa-rubric R5 기준:
   - 중첩 깊이 ≤3 권장
   - 불필요 래퍼 프레임 없음
5. **불일치 리포트** — 차이점 발견 시 목록으로 보고
...
| TDS에 동명 컴포넌트 존재 | 사용자에게 경고 → 업데이트 or 스킵 선택 요청 |
...
| 금지 | 이유 | 대안 |
|------|------|------|
| TDS 인스턴스를 기본 도형으로 재생성 | 스타일/바인딩/프로퍼티 유실 | `importComponentByKeyAsync` 사용 |
...
| Skill | 연계 시점 |
|-------|----------|
| `tds-naming-enforcer` | Phase 4 네이밍 위반 발견 시 |
| `tds-property-optimizer` | Phase 4 프로퍼티 위반 발견 시 |
| `tds-qa-auditor` | 생성 완료 후 전체 QA 실행 시 |
| `tds-cross-verify` | 프로덕트 파일에서 새 컴포넌트 사용 후 교차 검증 시 |
| `tds-review` | 컴포넌트화 후보 발견 → 이 스킬로 생성 (발견 경로) |

> **Post-workflow**: 생성 완료 후 TDS 라이브러리에서 **Publish** 필요. Publish는 Figma UI에서 수동 실행.
