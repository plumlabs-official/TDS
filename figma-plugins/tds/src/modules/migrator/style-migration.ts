 ...
 *
 * Source: migrate-to-tds/code.js processNode() (lines 589-846)
 */

import { findTdsVariable, findNearestColor, serializeEffects, findTdsEffectStyle, findNearestTdsEffect, findNearestTextStyle } from '../../shared/matching';
...
/**
 * Process a single node: replace fills/strokes/effects/text styles with TDS equivalents.
 */
    ...
    try {
      ...
      if (currentEffectStyle && currentEffectStyle.name.indexOf('focus/') !== 0) {
        ...
        if ((currentEffectStyle as EffectStyle).remote) {
          let tdsEffectStyle = findTdsEffectStyle(currentEffectStyle.name, ctx.effectByName, ctx.effectByShortName);

          // 이름 매칭 실패 → 속성 정확 매칭 → 근사 매칭 fallback
          if (!tdsEffectStyle || tdsEffectStyle.id === (node as any).effectStyleId) {
            ...
            if (nodeEffectKey && ctx.effectStyleByProps[nodeEffectKey]) {
              tdsEffectStyle = ctx.effectStyleByProps[nodeEffectKey];
            }
          }
          if (!tdsEffectStyle || tdsEffectStyle.id === (node as any).effectStyleId) {
            const nearest = findNearestTdsEffect((currentEffectStyle as EffectStyle).effects, ctx.tdsEffectStyles);
            if (nearest) {
              tdsEffectStyle = nearest;
          ...
          if (tdsEffectStyle && tdsEffectStyle.id !== (node as any).effectStyleId) {
            await (node as any).setEffectStyleIdAsync(tdsEffectStyle.id);
            ctx.stats.effects++;
        ...
        if (bv && bv.fills && bv.fills[fi]) {
          ...
          try {
            ...
            if (currentFillVar) {
              ...
              const tdsFillVar = findTdsVariable(currentFillVar.name, ctx.variableByFullName, ctx.variableByShortName);
              if (tdsFillVar && tdsFillVar.id !== currentFillVar.id) {
                fill = figma.variables.setBoundVariableForPaint(fill as SolidPaint, 'color', tdsFillVar);
                fillChanged = true;
                ctx.stats.fills++;
        ...
        if (bv && bv.strokes && bv.strokes[si]) {
          ...
          try {
            ...
            if (currentStrokeVar) {
              ...
              const tdsStrokeVar = findTdsVariable(currentStrokeVar.name, ctx.variableByFullName, ctx.variableByShortName);
              if (tdsStrokeVar && tdsStrokeVar.id !== currentStrokeVar.id) {
                stroke = figma.variables.setBoundVariableForPaint(stroke as SolidPaint, 'color', tdsStrokeVar);
                strokeChanged = true;
                ctx.stats.strokes++;
