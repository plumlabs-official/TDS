 ...
 *
 * Source: migrate-to-tds/code.js handleMigrate() (lines 229-871)
...
export async function handleMigrate(): Promise<string> {
  ...
  const tdsEffectStyles: EffectStyle[] = [];
  ...
  for (let es = 0; es < tdsEffectStyles.length; es++) {
    const eStyle = tdsEffectStyles[es];
