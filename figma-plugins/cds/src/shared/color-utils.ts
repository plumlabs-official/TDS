 ...
 *
 * Source: cds-docs/code.js (superior version with cross-collection alias fix)
 * Bug fix: Migrator's resolveColorValue used modes[0] — now uses findLightModeId
 */

import { getCdsVars } from './variables';
...
/**
 * Create a SolidPaint with CDS variable binding. Falls back to provided color.
 */
export function bindPaint(color: RGB, varName: string): SolidPaint {
  let paint: SolidPaint = { type: 'SOLID', color };
  const v = getCdsVars()[varName];
  if (v) {
    try {
