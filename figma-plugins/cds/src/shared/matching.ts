 ...
 *
 * Source: migrate-to-cds/code.js (lines 333-544)
...
/**
 * Find the nearest CDS color variable by Euclidean distance in RGBA space.
...
/**
 * Find a CDS variable by name (full path or short name fallback).
 */
export function findCdsVariable(
  originalName: string,
...
/**
 * Find CDS effect style by name (full or short name).
 */
export function findCdsEffectStyle(
  originalName: string,
...
/**
 * Find nearest CDS effect style by Euclidean distance in shadow property space.
 */
export function findNearestCdsEffect(
  ...
  for (const cds of cdsEffectStyles) {
    const tgtVec = extractEffectVector(cds.effects);
    if (!tgtVec) continue;
    const dist = Math.sqrt(
...
/**
 * Find nearest CDS text style by fontSize + fontWeight + lineHeight.
