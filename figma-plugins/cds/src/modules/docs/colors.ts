 ...
 * Source: cds-docs/code.js lines 365-601
 */
...
import { ensureVarsLoaded, getModeCollection, getAllModeVariables, getCdsVars } from '../../shared/variables';
...
export async function handleGenColors(): Promise<void> {
  ...
  const cdsVars = getCdsVars();
  const ringVar = cdsVars['ring'] || null;
  ...
  const subtitle = makeLabel('CDS Mode Collection — Semantic Color Tokens', 16, 'muted-foreground');
