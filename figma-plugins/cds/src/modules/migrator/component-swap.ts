/**
 * Component Instance Swap — Replace remote component instances with local CDS ones
 *
 * Source: migrate-to-cds/code.js swapComponentInstances() + findVariantInSet() + traverseAndSwap()
 * (lines 875-1055)
 */

import { CDS_PAGE_IDS } from '../../constants';
...
/**
 * Swap remote component instances with local CDS components.
 * Builds a component index from CDS source pages, then traverses target nodes.
 *
 * 3-tier matching:
...
export async function swapComponentInstances(
  ...
  const sourcePageIds = [CDS_PAGE_IDS.COMPONENTS, CDS_PAGE_IDS.ICONS];
  ...
  console.log('CDS components: ' + (compCount - setNames.length) + ', SETs: ' + setNames.length);
