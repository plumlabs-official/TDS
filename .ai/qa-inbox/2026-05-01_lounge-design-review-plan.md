...
- `Challenge List Screen` has category chips and list content, but not the full shared search/sort/pin toolbar decided in D4.
...
| D3 | Add Creator Ranking and Challenge Ranking; ranking criteria are not yet defined. | Wave 1 |
...
| D7 | Empty lounge state should shrink guidance and strengthen curation exposure. | Wave 2 |
...
| D10 | Paywall should use white/simple background and emphasize selected state only. | Backlog unless paywall frames are later selected |
| D11 | INVITE+ entry needs a separate special design; naming is provisional. | Backlog unless invite frames are later selected |
| D12 | CP recharge model is confirmed; details remain for later usability pass. | Backlog, no UI invention |
...
2. **Wave 1: Discover Tab**
   ...
   - Add/clarify Creator Ranking and Challenge Ranking sections without inventing ranking algorithms.
   ...
   - End checkpoint: screenshot export to `exports/2026-05-01_lounge-design-review/wave-1/`; if visual structure regresses or D1-D3 are not visible, revert this wave before continuing.

3. **Wave 2: My Tab**
   - Compress empty guidance into a small informative block.
   - Add Favorite Lounges entry/section for fast access.
   - Prioritize curated lounge cards immediately below the empty state.
   - End checkpoint: screenshot export to `exports/2026-05-01_lounge-design-review/wave-2/`; if D5/D7 are not visible, revert this wave before continuing.

4. **Wave 3: Challenge List + ChallengeTalk Tooling**
   ...
   - End checkpoint: both target screens must show the same tool pattern; otherwise stop and repair before Wave 4.
...
5. **Wave 4: Chat Preview Format**
   ...
   - End checkpoint: `Chat Preview Format` QA confirms avatar group is the primary identity and row bounds/touch targets pass.
...
- Figma mutations must run sequentially: Wave 1 -> QA checkpoint -> Wave 2 -> QA checkpoint -> Wave 3 -> QA checkpoint -> Wave 4 -> QA checkpoint -> Wave 5 only if needed -> Wave 6 final QA.
...
Proceed in waves, starting with `Discover Lounge Screen` and `My Lounge Screen`, then apply shared toolbar and chat preview changes to the related list screens. Do not redesign paywall, INVITE+, CP, or grade badge surfaces in this target canvas unless those frames are explicitly selected later; keep them as backlog decisions from the meeting.
