...
| **Rev. 16 — CDS 원본 그루핑 재편 (현재 세션)** | 사용자 피드백 "다 섞여있어서 보기 힘들어". CDS Figma 페이지 구조(`Primitives` 24 sub-group + `Composed` 20 sub-group) 그대로 Pencil에 복제. 각 sub-group frame = label header + reusable row. 133 reusables을 해당 sub-group으로 M() 이동 (6 batch). Dialog/Drawer → Card and Sheet, Footer → Navbar, Featured Icon → Asset 분류. **133 유지**. |
